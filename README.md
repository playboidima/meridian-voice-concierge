...
FAQ-пошук гібридний. Backend створює локальні англійські embeddings з актуальних answer/question/category моделлю `BAAI/bge-small-en-v1.5`, зберігає їх у PostgreSQL через `pgvector` і виконує cosine similarity search. Англомовні запитання-підказки доповнюють індекс, але старі фіксовані semantic passages більше не використовуються. Embedding-модель працює локально на CPU і не потребує API-ключа.

Для англійських питань спочатку перевіряється покриття значущих тем у поточному FAQ. Синоніми описують поняття, а не доступність послуг. Якщо бракує теми (наприклад, dogs у загальному FAQ про Aurelia), кандидат відхиляється незалежно від схожості назви закладу. Lexical scoring порівнює окремо питання, відповідь і кожну підказку; бонусу за назву ресторану немає. Серед допустимих кандидатів сильний lexical-збіг має пріоритет, потім застосовується semantic retrieval. Якщо допустимого збігу вище відповідного порога немає, API повертає `matched: false`.

Для semantic результату також перевіряється відрив від другого кандидата. Якщо
два FAQ мають майже однакову оцінку, semantic-кандидат не приймається; допустимий
lexical-кандидат усе ще може пройти власний поріг. Це консервативний локальний
пошук, а не універсальна перевірка істинності: незнайомі перефразування можуть
потрапити в чергу навіть за наявності спорідненої інформації. Нові формулювання
слід перевіряти позитивними й негативними тестами, а не просто знижувати пороги.

### Оновлення пошукового індексу

Створення та редагування FAQ автоматично перераховують його embedding. Після
оновлення коду формування індексу на вже наявній базі виконайте:

```powershell
docker compose exec backend python -m app.reindex_faqs
```

Перед цим зробіть backup. Команда обробляє лише поточні записи: не запускає seed,
не відновлює видалені FAQ, не змінює їхні тексти, категорії, ID чи timestamps.
Операція транзакційна: помилка відкочує всі вектори. На час виконання записи
заблоковані від конкурентного редагування; за поточного малого каталогу це
коротка операція. Reindex не запускається автоматично на кожному старті.
...
Під час першого Docker build завантажується локальна embedding-модель. Backend автоматично застосує Alembic-міграції та увімкне `pgvector`. На новій базі початкові 48 FAQ з embeddings додаються лише один раз. Повторні запуски не переписують відповіді, не відновлюють видалені FAQ й не повертають попередні назви. API буде доступний на `http://localhost:8000`, Swagger — на `http://localhost:8000/docs`, React-адмінпанель — на `http://localhost:3000`.

### Збереження FAQ під час оновлення

Міграція `20260827_05` створює постійну позначку початкового наповнення. Для наявної інсталяції вона позначає наповнення виконаним, не змінюючи FAQ — навіть якщо всі FAQ були видалені. Для нової бази, створеної одним `alembic upgrade head`, FAQ і позначка зберігаються в одній транзакції. Помилку наповнення можна безпечно повторити; одночасні seed-процеси серіалізуються блокуванням рядка.

Перед оновленням наявної бази зробіть локальний `pg_dump` поза репозиторієм. Міграція потребує живого підключення до PostgreSQL (режим `--sql` не підтримується). База, яка вже мала Alembic-версію до оновлення, вважається наявною: навмисно порожній каталог не заповнюється автоматично. Зміни `seed_data.py` після першого наповнення не публікуються в існуючу базу; редагуйте FAQ через адмінпанель або застосовуйте окрему погоджену міграцію контенту. Не скидайте позначку наповнення для звичайного перезапуску.

Регресійні тести `backend/tests/test_seed_persistence_postgres.py` і `test_reindex_postgres.py` запускаються лише з `SEED_TEST_DATABASE_URL`, спрямованим на окрему тимчасову базу з назвою `meridian_seed_test`. Користувач тестової PostgreSQL має право створення баз: кожен тест створює й видаляє тільки власну базу `seed_test_<uuid>`. Тести перевіряють нову інсталяцію, оновлення наявної/порожньої бази, редагування, видалення, перейменування, помилку, паралельні запуски та reindex. Без цієї змінної тести пропускаються; не спрямовуйте їх на робочу базу.

`agent/tests/test_search_recording_integration.py` використовує `SEARCH_TEST_BACKEND_URL` лише для ізольованого хоста `meridian-search-audit-backend`. Він перевіряє справжній шлях search → no-match → unanswered і frequency; ці тестові записи не призначені для робочої бази.
...
- `422 Unprocessable Entity` — JSON не відповідає правилам полів, наприклад
  ...
  1000 символів. Відповідь містить стандартний масив FastAPI `detail` з
...
статус вихідного запитання на `converted`. Адмінпанель одразу додає повернений FAQ
до FAQ Library і прибирає питання з черги без перезавантаження сторінки.
Питання підтримують до 1000 символів включно в API, формі та PostgreSQL.
Міграція `20260827_06` розширює поле без зміни даних; зворотна міграція
відмовляється працювати за наявності питань довших за 500 символів, а не обрізає їх.
Відхилити нерелевантне запитання:
...
Остання автоматична перевірка після аудиту (27 August 2026): Backend 142,
Agent 16, Admin 17 тестів — усі пройшли без пропусків. Чистий запуск і
повторний старт перевірені в окремому Compose-проєкті. Деталі, ручні
голосові перевірки й обмеження наведено в `CORE_ACCEPTANCE.md`.

Назву голосового агента визначає backend через `AGENT_NAME` (типово
`meridian-concierge`). Docker Compose передає однакове значення backend і worker.
Playground не задає власну назву: сервер включає в токен рівно один dispatch
для налаштованого агента, навіть якщо старий клієнт передав іншу назву.
Після зміни `AGENT_NAME` потрібно перестворити backend і agent зі спільним `.env`;
перезбирати frontend лише заради зміни назви не потрібно.
...
Усі чотири сервіси мають бути `healthy`. На новій базі кількість FAQ — `48`; у вже використаній базі вона залежить від ваших додавань і видалень.
